...
def merge_similar_lines(l, lines):
    ...
    d = np.column_stack(((lines[:, 0] + lines[:, 2]) / 2, (lines[:, 1] + lines[:, 3]) / 2))
    d = np.tile(d, (1, 2))
    tl = l - d
    ...
    cos_theta = xd / td
    sin_theta = yd / td
    tl = np.column_stack((tl[:, 0] * cos_theta + tl[:, 1] * sin_theta,
    ...
    xb = np.sqrt((lines[:, 0] - lines[:, 2]) ** 2 + (lines[:, 1] - lines[:, 3]) ** 2) / 2 + 10
    yb = 15
...
def is_edge_line(l, dims):
    tol = 10
    bad = (l[0] < tol and l[2] < tol) or (l[1] < tol and l[3] < tol) or \
        (l[0] > dims[1]-tol and l[2] > dims[1]-tol) or (l[1] > dims[0]-tol and l[3] > dims[0]-tol)
    return bad
        ...
        if n1 in line_dict:
            for n2 in rel_dict[n1]:
                if n2 in line_dict:
                    ...
                    A = np.vstack((A,[[u[0],u[1],1,0,0,0,-u[0]*x[0],-u[1]*x[0]],[0,0,0,u[0],u[1],1,-u[0]*x[1],-u[1]*x[1]]]))
...
class SoccerModel:
    ...
    def predict(self, frame):
        ...
        if frame == 1:  # or frame drastically changed:
            ...
            obj.loc[valid_old, ['xnew', 'ynew']] = 0.5 * obj.loc[valid_old, ['xold', 'xold']] + 0.5 * obj.loc[
                valid_old, ['xnew', 'xnew']]

            obj['vel'] = np.linalg.norm(obj.loc[:, ['xnew', 'ynew']].values - df.loc[:, ['xold', 'xold']].values,
                                        axis=1) / 30  # fps

        self.obj_old = obj

        vals = [(0, (i['left'], i['top']), (i['left'] + i['width'], i['top'] + i['height']), i['vel'], i['xnew'], i['ynew']) for i in obj.iterrows()]
